...
const TOOL_TEXT = {
  ...
  todo_write: () => 'Use a structured task list only when multi-step or parallel state would otherwise be easy to lose. Do not create todos for straightforward work or mirror every action. Every call sends the ENTIRE list; update only when task state materially changes. Statuses: `pending`, `in_progress`, `completed`.',
  ...
  read_image: () => 'Inspect a PNG/JPEG/WebP/GIF directly. For a visual question, use the available image before inferring from code, geometry or proxy metrics, and inspect a comparable image after the fix. Paths without an extension work; format is detected from content. Oversized images are downscaled automatically — do not build thumbnails just to inspect them. Requires image input.',
