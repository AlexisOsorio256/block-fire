...
const TOOL_TEXT = {
  bash: (upstream, escalation) => {
    ...
    return 'Execute a bash command (`bash -c`) and return stdout/stderr. Each call is a fresh shell — pass `workdir` instead of `cd`. Non-zero exits report `[exit code: N]`; investigate failures. Managed `$DSH_*` variables carry harness facts. Long output is truncated to its tail, with the full output path reported when available. After a visual sweep, the first code/search/probe call commits to one owner: bash may then serve only that owner or one patch-selecting causal check before editing. A `TERMINAL VISUAL PASS` is final; do not retry another tool.' + denial + background + sandbox
  },
  ...
  read_image: () => 'Inspect PNG/JPEG/WebP/GIF directly. Batch related views: four or more belong in one readable sheet/strip; two or three independent views belong in one assistant step. Keep overview sheets compact, normally 8-12 semantic tiles. One focused detail may clarify a candidate before owner commitment. Once any code/search/probe call follows the visual sweep, EVIDENCE LOCK applies and no more images are allowed until after a real edit. If no player-visible defect is nameable, stop instead of reconstructing the images from memory. Use images before inferring from code, geometry or proxy metrics; after editing inspect comparable evidence.',
