...
const TOOL_TEXT = {
  bash: (upstream, escalation) => {
    ...
    return 'Execute a bash command (`bash -c`) and return stdout/stderr. Each call is a fresh shell — pass `workdir` instead of `cd`. Non-zero exits report `[exit code: N]`; investigate failures. Managed `$DSH_*` variables carry harness facts. Long output is truncated to its tail, with the full output path reported when available. In visual work after EVIDENCE LOCK, bash is only for the owner or one patch-selecting causal check before editing; do not generate more discovery QA.' + denial + background + sandbox
  },
  ...
  read_image: () => 'Inspect PNG/JPEG/WebP/GIF directly. Batch related views: four or more belong in one sheet/strip; two or three independent views belong in one assistant step. One overview is discovery. Once it makes a player-visible defect nameable and an owner plausible, EVIDENCE LOCK applies: read no more images until after the edit unless one hidden detail must choose between materially different patches. Use images before inferring from code, geometry or proxy metrics; after editing inspect comparable evidence.',
