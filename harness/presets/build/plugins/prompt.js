...
const TOOL_TEXT = {
  ...
  read_image: () => 'Inspect PNG/JPEG/WebP/GIF directly. For four or more related images, serial one-at-a-time reads are waste: inspect one contact sheet/strip first, then open individuals only for hidden detail or continuity. For two or three independent views, read them concurrently in one assistant step. Scan the whole overview and inventory visible defects before focusing. Use images before inferring from code, geometry or proxy metrics; inspect comparable after evidence. Oversized images downscale automatically.',
