...
## --aim measures unresolved barrel/grip issues; --baseline reports without failing.
var failures := 0
func _init() -> void:
	call_deferred("run")
...
func run() -> void:
	...
	# Opt-in unresolved visual diagnostic; the default gate verifies the FOV fix.
	if "--aim" in OS.get_cmdline_user_args():
		for index in 4:
			...
			for pitch: float in [-45, 0, 45]:
				...
				print("AIM weapon=%s pitch=%.0f error_deg=%.6f socket_mm=%.6f" % [id, pitch, error, socket_error * 1000])
			...
			for state: String in ["strafe", "sprint_ads", "reload", "shot"]:
				player.look_pitch = 45.0
				var max_grip := 0.0
				var max_flip := 0.0
				var previous := {"L": Vector3.ZERO, "R": Vector3.ZERO}
				controls.qa_set_move(Vector2.RIGHT if state == "strafe" else (Vector2.UP if state == "sprint_ads" else Vector2.ZERO))
